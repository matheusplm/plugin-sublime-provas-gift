...
def numberquestions(self, edit):
    ...
    for rgn in self.view.find_all("Resposta"):
        self.view.sel().add(rgn)

    for rgn in self.view.find_all("RESPOSTA CORRETA"):
        self.view.sel().add(rgn)

    for rgn in self.view.find_all("RESPOSTA CERTA"):
        self.view.sel().add(rgn)
    ...
    #Procurar todos as falhas descritas com ~ }
    for rgn in self.view.find_all("~ \}"):
        self.view.sel().add(rgn)
    ...
    for sel in sels:
        ...
        string = ""
        self.view.replace(edit, sel, string)
    #fim da função de achar os bragulho


    # find all opening curly braces
    #CONTINUAR MEXENDO DAQUI AAAAAAAA
    for rgn in self.view.find_all('^\s*$[\n]'):
        self.view.sel().add(rgn)
        count += 1
    ...
    for sel in sels:
        ...
        if string.find('CATEGORY:') != -1:
            ...
            if questao < 10:
                squestao = "0" + str(questao)
            else:
                squestao = str(questao)
            string = "\n::" + prefix + "Questão: " + squestao + ":: " + string
            self.view.replace(edit, sel, string)    
            questao += 1
